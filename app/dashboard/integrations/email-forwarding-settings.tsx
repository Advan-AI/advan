  ...
  return (
    <DashCard title="Email Inbound Channel" icon={<Mail className="w-[18px] h-[18px]" />} padded>
      <div className="space-y-6 max-w-2xl">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1 flex items-center justify-between border dash-border-soft rounded-lg px-3.5 py-2.5 bg-white shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)]">
              <span className="text-[13.5px] font-mono select-all text-[var(--dash-ink-soft)] tracking-tight font-semibold">
                {inboundEmailAddress}
              </span>
              <button
                type="button"
                onClick={handleCopy}
                className="text-[var(--dash-ink-faint)] hover:text-[var(--dash-ink)] transition-colors ml-3 p-1 rounded hover:bg-[var(--dash-bg-deep)]"
                title="Copy address"
              >
        ...
        <div className="rounded-xl border border-amber-500/10 bg-amber-500/[0.02] p-4 space-y-3">
          <div className="flex items-start gap-2.5">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11.5px] text-amber-800/80 leading-normal">
                To keep your public support address looking clean (e.g., <code>support@yourdomain.com</code>), configure automatic forwarding rules inside your email provider.
              </p>
            </div>
          </div>
          <ul className="text-[11px] text-[var(--dash-ink-soft)] space-y-1.5 list-disc pl-5 leading-normal">
            <li>
              <strong>Google Workspace (Gmail):</strong> Go to Gmail Admin &gt; Routing &gt; Add a rule to forward incoming mail to <code>{inboundEmailAddress}</code>.
            </li>
            <li>
              <strong>Microsoft 365 (Exchange):</strong> Go to Exchange Admin Center &gt; Mail Flow &gt; Rules &gt; Add a rule to redirect support messages to <code>{inboundEmailAddress}</code>.
            </li>
          </ul>
        </div>
      </div>
